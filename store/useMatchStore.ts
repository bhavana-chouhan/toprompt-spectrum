...
import { matchs as seedMatchs } from '@/services/mock-data';

const RESOURCE_PATH = '/api/matches';
...
interface MatchStore {
  ...
  fetchNextMatchs: () => Promise<void>;
  addMatch: (input: Partial<Match>) => Promise<Match>;
  /** Alias of addMatch kept for screens that call the older name. */
  addOptimisticMatch: (input: Partial<Match>) => Promise<Match>;
  updateMatch: (id: string, patch: Partial<Match>) => Promise<void>;
  deleteMatch: (id: string) => Promise<void>;
  toggleField: (id: string, field: keyof Match) => Promise<void>;
  getMatchById: (id: string) => Match | null;
}

/**
 * UNIFIED dual-phase Zustand store for Match.
 *
 ...
 *
 *   1. The `import { matchs as seedMatchs } from '@/services/mock-data'`
 *      line is stripped (mock-data.ts is deleted in Phase 2).
 *   2. The `matchs: (seedMatchs as Match[]) ?? []` initializer
 *      collapses to `matchs: []` once `seedMatchs` is undefined.
 *      The `?? []` keeps both phases safe; the codemod additionally rewrites
 *      `seedMatchs` references in the body to `undefined` for clarity.
 *
 ...
 *      (`match_<ts>_<rand>`) so optimistic UI has a stable key. That
 ...
 *        "Cast to ObjectId failed for value \"match_1779...\""
 ...
 *      ObjectId references; for Match those are user1, user2.
...
const __RELATION_FIELDS: string[] = ["user1","user2"];
const __normalizeForWire = (
  input: Record<string, unknown> | Partial<Match>,
...
export const useMatchStore = create<MatchStore>((set, get) => ({
  ...
  fetchMatchs: async () => {
    ...
    try {
      const items = await apiGet<Match[]>(RESOURCE_PATH);
      set({
        matchs: Array.isArray(items) ? items : get().matchs,
        loading: false,
      });
    } catch (err) {
  ...
  addMatch: async (input) => {
    ...
    try {
      ...
      set((s) => ({
        matchs: s.matchs.filter((x) => x._id !== optimistic._id),
  ...
  addOptimisticMatch: async (input) => {
    ...
    // addMatch is ALREADY optimistic.
    return get().addMatch(input);
  },

  updateMatch: async (id, patch) => {
    const prev = get().matchs.find((x) => x._id === id);
    if (!prev) return;
    const optimistic: Match = { ...prev, ...patch, updatedAt: new Date().toISOString() };
    set((s) => ({
      matchs: s.matchs.map((x) => x._id === id ? optimistic : x),
    }));
    if (IS_DESIGN_PREVIEW) {
    ...
    try {
      ...
      const updated = await apiPatch<Match>(RESOURCE_PATH + '/' + id, __normalizeForWire(patch));
      if (updated && (updated as { _id?: string })._id) {
        set((s) => ({
          matchs: s.matchs.map((x) => x._id === id ? updated : x),
        }));
      }
    } catch (err) {
      if (__isNetworkError(err)) {
      ...
      set((s) => ({
        matchs: s.matchs.map((x) => x._id === id ? prev : x),
  ...
  deleteMatch: async (id) => {
    ...
    if (!prev) return;
    set((s) => ({
      matchs: s.matchs.filter((x) => x._id !== id),
    }));
    if (IS_DESIGN_PREVIEW) {
  ...
  toggleField: async (id, field) => {
    ...
    await get().updateMatch(id, { [field]: nextVal } as Partial<Match>);
  },

  getMatchById: (id) => get().matchs.find((x) => x._id === id) ?? null,
}));
