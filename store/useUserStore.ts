...
import { users as seedUsers } from '@/services/mock-data';

const RESOURCE_PATH = '/api/users';
...
interface UserStore {
  ...
  addUser: (input: Partial<User>) => Promise<User>;
  /** Alias of addUser kept for screens that call the older name. */
  addOptimisticUser: (input: Partial<User>) => Promise<User>;
  updateUser: (id: string, patch: Partial<User>) => Promise<void>;
  deleteUser: (id: string) => Promise<void>;
  toggleField: (id: string, field: keyof User) => Promise<void>;
  getUserById: (id: string) => User | null;
}

/**
 * UNIFIED dual-phase Zustand store for User.
 *
 ...
 *
 *   1. The `import { users as seedUsers } from '@/services/mock-data'`
 *      line is stripped (mock-data.ts is deleted in Phase 2).
 *   2. The `users: (seedUsers as User[]) ?? []` initializer
 *      collapses to `users: []` once `seedUsers` is undefined.
 *      The `?? []` keeps both phases safe; the codemod additionally rewrites
 *      `seedUsers` references in the body to `undefined` for clarity.
 *
 ...
 *      (`user_<ts>_<rand>`) so optimistic UI has a stable key. That
 ...
 *        "Cast to ObjectId failed for value \"user_1779...\""
 ...
 *      ObjectId references; for User those are (none).
...
const __RELATION_FIELDS: string[] = [];
const __normalizeForWire = (
  input: Record<string, unknown> | Partial<User>,
...
export const useUserStore = create<UserStore>((set, get) => ({
  ...
  fetchUsers: async () => {
    ...
    try {
      const items = await apiGet<User[]>(RESOURCE_PATH);
      set({
        users: Array.isArray(items) ? items : get().users,
        loading: false,
      });
    } catch (err) {
  ...
  addUser: async (input) => {
    ...
    try {
      ...
      const created = await apiPost<User>(RESOURCE_PATH, __normalizeForWire(input));
      ...
      set((s) => ({
        users: s.users.filter((x) => x._id !== optimistic._id),
  ...
  addOptimisticUser: async (input) => {
    ...
    // addUser is ALREADY optimistic.
    return get().addUser(input);
  },

  updateUser: async (id, patch) => {
    const prev = get().users.find((x) => x._id === id);
    if (!prev) return;
    const optimistic: User = { ...prev, ...patch, updatedAt: new Date().toISOString() };
    set((s) => ({
      users: s.users.map((x) => x._id === id ? optimistic : x),
    }));
    if (IS_DESIGN_PREVIEW) {
    ...
    try {
      ...
      const updated = await apiPatch<User>(RESOURCE_PATH + '/' + id, __normalizeForWire(patch));
      if (updated && (updated as { _id?: string })._id) {
        set((s) => ({
          users: s.users.map((x) => x._id === id ? updated : x),
        }));
      }
    } catch (err) {
      if (__isNetworkError(err)) {
      ...
      set((s) => ({
        users: s.users.map((x) => x._id === id ? prev : x),
  ...
  deleteUser: async (id) => {
    ...
    if (!prev) return;
    set((s) => ({
      users: s.users.filter((x) => x._id !== id),
    }));
    if (IS_DESIGN_PREVIEW) {
  ...
  toggleField: async (id, field) => {
    ...
    await get().updateUser(id, { [field]: nextVal } as Partial<User>);
  },

  getUserById: (id) => get().users.find((x) => x._id === id) ?? null,
}));
